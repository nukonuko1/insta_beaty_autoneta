...
function firstDayOfMonthMon(year: number, month: number) {
  // 0=Mon … 6=Sun
...
export default function CalendarView({ scheduledPosts, onUpdate }: Props) {
  ...
  const offset = firstDayOfMonthMon(year, month);
  const monthStr = `${year}-${pad(month + 1)}`;
  const todayStr = `${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())}`;

  // Map: "YYYY-MM-DD" → ScheduledPostEntry[]
  const byDate = scheduledPosts.reduce<Record<string, ScheduledPostEntry[]>>(
    (acc, entry) => {
  ...
  const scheduledCount = Object.keys(byDate).length;
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between bg-white rounded-2xl border border-purple-100 px-4 py-3">
        ...
        <div className="text-center">
          ...
          {scheduledCount > 0 && (
            <p className="text-xs text-purple-500">{scheduledCount}日に投稿予定</p>
          )}
        </div>
        <button
          type="button"
      ...
      <div className="bg-white rounded-2xl border border-purple-100 overflow-hidden">
        ...
        <div className="grid grid-cols-7">
          ...
          {Array.from({ length: offset }).map((_, i) => (
            <div
              key={`off-${i}`}
              className="h-12 border-b border-r border-purple-50/50"
            />
          ))}
      ...
      {selectedDate && (
        <div>
          ...
          {(byDate[selectedDate] ?? []).length === 0 ? (
            <p className="text-gray-400 text-sm text-center py-6 bg-white rounded-2xl border border-purple-100">
              この日に予定された投稿はありません
            </p>
          ) : (
