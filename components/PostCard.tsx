...
export default function PostCard({ post, index, onUpdate }: PostCardProps) {
  ...
  const currentStatus =
    STATUS_OPTIONS.find((s) => s.value === post.status) ?? STATUS_OPTIONS[0];
    ...
    >
      ...
      <div className="bg-gradient-to-r from-purple-500 to-pink-400 px-4 py-3 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span
            className={`text-xs font-medium px-2 py-0.5 rounded-full shrink-0 ${currentStatus.color}`}
          >
            {currentStatus.label}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {onUpdate && (
            <button
              type="button"
              onClick={() => onUpdate({ favorite: !post.favorite })}
        ...
        {onUpdate && (
          <div className="pt-2 border-t border-gray-100 flex flex-wrap gap-2">
            <select
              value={post.status}
              onChange={(e) =>
                onUpdate({ status: e.target.value as Post["status"] })
              }
            ...
            {showDate ? (
              <input
                ...
                autoFocus
                onChange={(e) => {
                  onUpdate({ scheduledDate: e.target.value || null });
                  setShowDate(false);
                }}
