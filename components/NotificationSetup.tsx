...
interface NotifSettings { enabled: boolean; hour: number; notifiedDate: string; }
const DEFAULT: NotifSettings = { enabled: false, hour: 18, notifiedDate: "" };

export default function NotificationSetup() {
  const [settings, setSettings] = useLocalStorage<NotifSettings>("ig_notif_settings", DEFAULT);
  ...
  useEffect(() => {
    if (typeof Notification === "undefined" || !settings.enabled || permission !== "granted") return;
    const check = () => {
      const now = new Date();
      ...
      if (now.getHours() >= curr.hour) {
        new Notification("Instagram投稿の時間です！", { body: "今日の投稿文を作りましょう。お店の最新情報をシェアしよう✨", icon: "/favicon.ico" });
    ...
    <div className="bg-white rounded-2xl border border-purple-100 p-4 mb-4">
      <button type="button" onClick={() => setExpanded((v) => !v)} className="w-full flex items-center justify-between text-left">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-sm font-semibold text-gray-800">毎日の投稿リマインダー</p>
            <p className="text-xs text-gray-400">{isActive ? `毎日 ${settings.hour}:00 に通知します` : "投稿習慣をサポートします"}</p>
          </div>
        </div>
          ...
          {permission !== "granted" ? (
            ...
            <>
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={() => setSettings((prev) => ({ ...prev, enabled: !prev.enabled }))}
                  className={`w-12 h-6 rounded-full transition-colors relative ${settings.enabled ? "bg-purple-500" : "bg-gray-300"}`}>
                  <span className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${settings.enabled ? "translate-x-7" : "translate-x-1"}`} />
                </button>
              </div>
              {settings.enabled && (
                <div className="flex items-center gap-3">
                  ...
                  <select value={settings.hour}
                    onChange={(e) => setSettings((prev) => ({ ...prev, hour: Number(e.target.value), notifiedDate: "" }))}
